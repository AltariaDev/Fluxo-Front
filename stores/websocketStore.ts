...
export const useWebSocketStore = create<WebSocketStore>((set, get) => ({
  ...
  actions: {
    initSocket: (token: string) => {
      ...
      socket.on("status", (status: PomodoroStatusType) => {
        ...
        if (
          ...
          useTimerStore.getState().actions.setStartedElement(true);
        }
        set({ status });
      });
    ...
    setIsConnected: (isConnected) =>
      set({ isConnected: isConnected as boolean }),
...
export const useSocket = () => useWebSocketStore((state) => state.socket);
export const useStatus = () => useWebSocketStore((state) => state.status);
